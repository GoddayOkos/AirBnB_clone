...
class HBNBCommand(cmd.Cmd):
    ...
    """
    prompt = '(hbnb) '
    __class_set = {"BaseModel", "User", "State", "City", "Amenity", "Place",
                    "Review"}
    ...
    def do_create(self, args):
        ...
        """
        line = args.split()
        if not self.__verify_class(line):
            return
    ...
    def do_all(self, args):
        ...
        elif line[0] in HBNBCommand.__class_set:
    ...
    def do_update(self, args):
        ...
        if not self.__verify_id(line):
            return
        if not self.__verify_attribute(line):
            return
        objects = models.storage.all()
        key = '{}.{}'.format(line[0], line[1])
    ...
    def __verify_class(self, line):
        """
        Private method to verify inputed class
        """
        if len(line) == 0:
            print('** class name missing **')
            return False
        elif line[0] not in HBNBCommand.__class_set:
            print('** class doesn\'t exist **')
            return False
        return True

    def __verify_id(self, line):
        """
        Private method to verify the id.
        """
        if len(line) < 2:
    ...
    def __verify_attribute(self, line):
        """
        Private method to verify the attribute in inputted line.
        """
        if len(line) < 3:
